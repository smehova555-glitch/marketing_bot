...
async def q_city(message: Message, state: FSMContext):
    ...
    await message.answer("В какой сфере работает ваш бизнес?",
                         reply_markup=kb(["Бьюти / Салон", "Эксперт / Онлайн", "Услуги", "E-commerce", "Производство", "Другое"]))
...
async def q_niche(message: Message, state: FSMContext):
    ...
    await message.answer("Есть ли маркетинговая стратегия?",
...
async def q_strategy(message: Message, state: FSMContext):
    ...
    await message.answer("Основной источник заявок?",
...
async def q_source(message: Message, state: FSMContext):
    ...
    await message.answer("Есть ли стабильный поток заявок?",
...
async def q_stability(message: Message, state: FSMContext):
    ...
    await message.answer("Есть ли карточка в Яндекс/2ГИС?",
...
async def q_geo(message: Message, state: FSMContext):
    ...
    await message.answer("Какой маркетинговый бюджет в месяц?",
